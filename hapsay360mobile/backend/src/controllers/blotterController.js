...
//Format Date for UI (e.g., "Oct 14, 10:45 AM")
...
//Generate Status Timeline based on DB Status
...
/* POST /submit Create a new blotter entry*/
...
/* GET /my-blotters Get blotters for the Main List View */
export const getMyBlotters = async (req, res) => {
...
/* GET /track/:number Get Details for the Modal (Includes Timeline Logic) */
...
/* DELETE /:id Delete blotter (user-owned only) */
export const deleteBlotter = async (req, res) => {
  ...
  if (!blotter) {
    return res.status(404).json({ message: "Blotter not found or not yours" });
  }
